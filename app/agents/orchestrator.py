...
class Orchestrator:
    def __init__(self, registry, vector_store, experience_store=None):
    ...
    def call_agent(self, state: AgentState) -> AgentState:
        ...
        models_to_try = ["models/gemini-3.6-flash", "models/gemini-3.5-flash", "models/gemini-2.5-flash", "models/gemini-pro-latest"]
        raw_response = ""
        last_err = None
    ...
    def run(self, user_message: str, session_id: str = "default") -> str:
        ...
        try:
            ...
            for msg in final_messages:
                if msg.get("action") and msg.get("action") != "reply":
                    tool_used = msg.get("action")
                    tool_input = msg.get("input")
                if msg.get("role") == "tool" or ("name" in msg and "result" in msg):
                    if not tool_used and "name" in msg:
                        tool_used = msg.get("name")
                    raw_res = msg.get("result")
                    if isinstance(raw_res, str):
                        try:
                            tool_result = json.loads(raw_res)
                        except Exception:
                            tool_result = {"output": raw_res}
            ...
            print(f"Warning: Orchestrator execution error: {e}")
        ...
        if self.store:
            try:
                ...
                self.store.log_experience(experience)
            except Exception as e:
                print(f"Warning: Failed to log experience: {e}")
